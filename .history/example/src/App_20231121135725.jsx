  ...
  function buildAdjMat() {
    ...
    for(let i = 0; i < subset.length; ++i) {
        for(let j = i + 1; j < subset.length; ++j) {
            const dist = distance(subset[i].lat, subset[i].lng, subset[j].lat, subset[j].lng);
            const dist2 = calcDistance(subset[i].lat, subset[i].lng, subset[j].lat, subset[j].lng);
  ...
  async function getData() {
    const data = Papa.parse(await fetchCsv(), {
      ...
      complete: function(results) {
        setArray((results.data.sort(() => 0.5 - Math.random())).filter((value) => value.country === "United States"));
      }
    });
    return data;
  }
